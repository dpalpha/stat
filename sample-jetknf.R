bad_var_estimator = function(x){
    n = length(x)
    return(var(x)*(n-1)/n)
#count bad statistic features
}


JN_bias_correction = function(x, estimator){
    n = length(x)
    theta_starts = vector("numeric", n)
    ind = c(1:n)
    for (i in ind){
        sample = x[ind != i]
        theta_starts[i] = estimator(sample)
    }
    theta_hat = estimator(n)
    theta_dot = mean(theta_starts)
...
df = data.frame(x=rep(sample_sizes, 3),
               ...
               dr = factor(rep(1:3, each = length(sample_sizes)),
                          labels=c("results_good", "results_bad", "results_correctes")))
ggplot(df, aes(x, y, col=dr)) +
geom_jitter()
